...
export default function CarDetailView({ car, onBack }: CarDetailViewProps) {
  ...
  return (
    <div className="min-h-screen bg-[#111315] text-[#F3F0EA] pb-12">
      ...
      {/* الصورة الرئيسية: حاوية بنسبة 4:3 جوال و16:10 شاشات كبيرة ≈ 1600×1100 */}
      <section className="relative w-full overflow-hidden bg-[#0a0a0a]">
        <div
          ...
          className="relative w-full aspect-[4/3] md:aspect-[16/10] max-h-[75vh] mx-auto flex items-center justify-center cursor-zoom-in"
        >
          {/* توهج بنفسجي خفيف يحوّل أي فراغ إلى جزء من التصميم */}
          <div
          ...
          <CarImage
            ...
            style={{ filter: 'brightness(1.02) contrast(1.05)', objectPosition: 'center center' }}
          />
        </div>
        ...
        <div className="absolute bottom-6 right-4 md:right-12 z-20 flex items-center gap-2 px-3 py-1.5 bg-[#111315]/80 backdrop-blur-md border border-[#8B5CF6]/30 rounded-full" dir="ltr">
      ...
      {/* Lightbox: contain بحدود الشاشة كاملة — السيارة بأقصى حجم دون قص */}
      {lightboxOpen && (
          ...
          >
            {allImages[lightboxIndex] ? (
              <img
                ...
                alt={car.nameAr}
                className="max-w-full max-h-full w-auto h-auto object-contain transition-transform duration-200 ease-out select-none"
                style={{ transform: `translate(${position.x}px, ${position.y}px) scale(${scale})`, objectPosition: 'center center' }}
                draggable={false}
              />
            ) : (
